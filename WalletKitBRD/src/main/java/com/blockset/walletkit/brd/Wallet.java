...
final class Wallet implements com.blockset.walletkit.Wallet {
    ...
    private final double ETH_MULTIPLIER = 0.60;
    ...
    @Override
    public @Nullable String getAddressFromScript(@Nullable String outputScript) {

        return core.getAddressFromScript(outputScript);
    }
    ...
    private void estimateLimit(boolean asMaximum,
        ...
        Amount amount;
        if(result.amount.getUnit().getName().equalsIgnoreCase("Ether")) {
            Optional<Double> amountDouble = result.amount.getDouble(result.amount.getUnit());
            if (amountDouble.isPresent()) {
                double amountDoubleAdjusted = amountDouble.get() * ETH_MULTIPLIER; // Reduce ETH so API estimateFee succeeds
                amount = Amount.create(amountDoubleAdjusted, Amount.create(result.amount).getUnit());
            } else {
                amount = Amount.create(result.amount);
            }
        } else {
            amount = Amount.create(result.amount);
        }
